...
def check_response(response):
    if response.status_code == 503:
        st.error("❌ השרת סגור זמנית לתחזוקה (Error 503). אנא נסו שוב מאוחר יותר.")
        return False
    elif response.status_code != 200:
        st.error(f"❌ שגיאה בתקשורת עם השרת (Status Code: {response.status_code})")
        return False
    return True
...
if submit:
    try:
                    ...
                    if not df_siri.empty:
                        ...
                        with col_charts:
                            with st.container(border=True):
                                ...
                                st.plotly_chart(px.line(avg_dur, x='hour', y='duration_minutes', markers=True),
                                                use_container_width=True)

                            with st.container(border=True):
                                st.markdown(f"### Ride Distribution - {selected_day}")
                                st.plotly_chart(px.histogram(filtered_siri, x='hour', nbins=24),
                                                use_container_width=True)
                    else:
        ...
        st.error("❌ לא ניתן להתחבר לשרת. בדקו את חיבור האינטרנט שלכם.")
    except requests.exceptions.Timeout:
        st.error("❌ השרת איטי מדי ולא הגיב בזמן. נסו שוב בעוד דקה.")
